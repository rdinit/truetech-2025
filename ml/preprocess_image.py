...
def get_dense_reg_caption(image):
    return run_example('<DENSE_REGION_CAPTION>', image)
...
def get_open_vocab_detection(image, prompt):
    results = run_example('<OPEN_VOCABULARY_DETECTION>', image, prompt)
...
def get_detecton_with_desc(image, prompt):
    pre_results = run_example('<CAPTION>', image)
...
def get_detecton_with_detailed_desc(image, prompt):
    pre_results = run_example('<DETAILED_CAPTION>', image)
    text_input = pre_results[task_prompt]
    results = run_example('<CAPTION_TO_PHRASE_GROUNDING>', text_input)
